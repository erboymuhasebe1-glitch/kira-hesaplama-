...
# İşyeri Geliri Beyan Durumu Kontrolü (Dahillik Kuralı)
# Kural: Toplam Brüt, Beyan Sınırını GEÇMEDİĞİ (Eşit veya Küçük) takdirde işyeri beyan edilmez.
beyana_dahil_isyeri = 0.0
if toplam_gelir_brut > beyan_siniri:
    beyana_dahil_isyeri = isyeri_brut
    isyeri_beyan_notu = "Beyana Dahil (Sınır Aşıldı)"
else:
    beyana_dahil_isyeri = 0.0
    isyeri_beyan_notu = f"{beyan_siniri:,.0f} TL Sınırı Aşılmadı (İşyeri Dahil Edilmedi)"

# İstisna Hesaplama (Mesken gelirinden düşülür)
...
c1.metric("Vergi Matrahı", f"{matrah:,.2f} TL")
c2.metric("İşyeri Stopajı", f"{kesilen_stopaj:,.2f} TL")
if net_odenecek > 0:
...
wa_msg = (
    ...
    f"*Mesken İstisnası:* {istisna_tutari:,.2f} TL\n"
